...
def aggregate_candidates_from_scored_paths(
    ...
    *,
    ...
    """Deduplicate candidate users from scored paths and summarize evidence by pattern."""
    pattern = scored_result.get("pattern")
    if not isinstance(pattern, str) or not pattern.strip():
        raise ValueError("scored_result.pattern must be a non-empty string.")
    _ensure_supported_candidate_pattern(pattern)
    ...
    for scored_path in scored_result.get("scores", []):
        if not isinstance(scored_path, dict):
            continue
        candidate_id = _extract_candidate_patient_id(scored_path, pattern)
        ...
        if isinstance(path_index, int):
            bucket["path_indices"].append(path_index)
...
def _extract_candidate_patient_id(
    scored_path: dict[str, Any],
    pattern: str,
) -> str | None:
    """Extract one candidate patient id from a scored path for the given pattern."""
    if pattern == PATIENT_TASKSET_TASK_GAME_TASK_TASKSET_PATIENT:
        return _extract_candidate_patient_id_from_patient_taskset_task_game_task_taskset_patient(
            scored_path
        )
    raise ValueError(f"Unsupported pattern for candidate aggregation: {pattern}")


def _ensure_supported_candidate_pattern(pattern: str) -> None:
    """Validate that the pattern has candidate-extraction support."""
    if pattern != PATIENT_TASKSET_TASK_GAME_TASK_TASKSET_PATIENT:
        raise ValueError(f"Unsupported pattern for candidate aggregation: {pattern}")


def _extract_candidate_patient_id_from_patient_taskset_task_game_task_taskset_patient(
    scored_path: dict[str, Any],
) -> str | None:
    """Extract the similar patient id from the PATIENT_TASKSET_TASK_GAME_TASK_TASKSET_PATIENT path."""
