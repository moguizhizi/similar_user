...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--patient-list-date",
        help=(
            "Load patient IDs from the auto-generated file for this date, using "
            "--patient-list-dir/base_DATE/patients_active_DATE.txt."
        ),
    )
    parser.add_argument(
        "--patient-list-dir",
        default=str(DEFAULT_PATIENT_IDS_OUTPUT_DIR),
        help="Base directory for patient ID files generated by export_patient_ids_with_training_on_date.py.",
    )
    parser.add_argument(
        "--base-date",
        required=True,
...
def build_patient_ids_file_from_date(
    *,
    patient_list_date: str,
    patient_list_dir: str | Path = DEFAULT_PATIENT_IDS_OUTPUT_DIR,
) -> Path:
    """Build the expected patient ID file path for an exported date list."""
    return build_patient_ids_output_path(
        base_date=patient_list_date,
...
def main() -> int:
    ...
    try:
        patient_source_count = sum(
            bool(value)
            for value in (
                args.patient_id,
                args.patient_ids_file,
                args.patient_list_date,
            )
        )
        if patient_source_count > 1:
            raise ValueError(
                "Use only one patient source: --patient-id, patient_ids_file, "
                "or --patient-list-date."
            )
        patient_ids_file = args.patient_ids_file
        if args.patient_list_date is not None:
            patient_ids_file = build_patient_ids_file_from_date(
                patient_list_date=args.patient_list_date,
                patient_list_dir=args.patient_list_dir,
            )
            if not Path(patient_ids_file).exists():
                export_patient_ids_with_training_on_date(
                    base_date=args.patient_list_date,
                    config_path=args.config,
                    output_dir=args.patient_list_dir,
                )
        if args.patient_id is not None:
            patient_ids = [args.patient_id]
        elif patient_ids_file is not None:
            patient_ids = read_patient_ids(patient_ids_file)
        else:
            patient_ids = None
        if not patient_ids:
            raise ValueError(
                "No patient IDs were provided. Use --patient-id, patient_ids_file, "
                "or --patient-list-date."
            )
