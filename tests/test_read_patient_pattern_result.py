...
class ReadPatientPatternResultScriptTest(unittest.TestCase):
    ...
    def test_stored_pattern_result_can_convert_unknown_paths_to_domain_objects(
        self,
    ) -> None:
        result = StoredPatternResult.from_dict(
            {
        ...
        self.assertEqual(len(domain_paths), 1)
        self.assertEqual(domain_paths[0].p.id, "30010096")
        self.assertEqual(domain_paths[0].un.name, "儿童相关-其他")
        self.assertEqual(domain_paths[0].p2.id, "20113562")

    def test_stored_pattern_result_can_convert_disease_taskset_patient_paths_to_domain_objects(
        self,
    ) -> None:
        result = StoredPatternResult.from_dict(
            {
        ...
        self.assertEqual(len(domain_paths), 1)
        self.assertEqual(domain_paths[0].d.id, "AU_DIS_0013")
    ...
    def test_stored_pattern_result_can_convert_unknown_taskset_patient_paths_to_domain_objects(
        self,
    ) -> None:
        result = StoredPatternResult.from_dict(
            {
        ...
        self.assertEqual(len(domain_paths), 1)
        self.assertEqual(domain_paths[0].un.id, "AU_UNKOWN_0005")
        self.assertEqual(domain_paths[0].s.训练日期, "2022-01-13")
        self.assertEqual(domain_paths[0].p.id, "30010096")
