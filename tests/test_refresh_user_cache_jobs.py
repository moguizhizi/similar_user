...
class RefreshUserCacheJobsTest(unittest.TestCase):
    ...
    def test_refresh_user_cache_jobs_rebuilds_patient_scored_paths_job(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            root = Path(temp_dir)
            config_path = _write_config(root)
            store = UserCacheIndexStore(root / "user_cache" / "cache_index.sqlite")
            ...
            with patch(
                "scripts.refresh_user_cache_jobs.score_and_save_configured_pattern_paths",
                return_value=[],
            ) as mock_score:
                summary = refresh_user_cache_jobs(
                    config_path=config_path,
    ...
    def test_refresh_user_cache_jobs_rebuilds_direct_entity_scored_paths_job(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            root = Path(temp_dir)
            config_path = _write_config(root)
            sqlite_path = root / "user_cache" / "cache_index.sqlite"
        ...
        mock_save_scored.assert_called_once_with(
            refreshed_score_result,
            root / "scored",
            config_path=config_path,
        )
